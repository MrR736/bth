...
static void print_usage(const char *progname) {
	LOGI("Usage: %s -i <input> -o <output> -f <name> [-c <count>] [Options]\n\n"
		"<Commands>\n"
...
static char *sanitize_name(const char *name) {
	...
	for (char *c = sanitized + 1; *c; c++) { if (!isalnum(*c) && *c != '_') *c = '_'; }
	return sanitized;
}

static char *uppercase_name(const char *name) {
...
int main(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
			...
			count = atoi(argv[++i]);
			if (count <= 0) {
				LOGE("Invalid count value");
				return EXIT_FAILURE;
			}
		} else if (strcmp(argv[i], "-f") == 0 && i + 1 < argc) { function_name = argv[++i]; }
		else if (strcmp(argv[i], "-i") == 0 && i + 1 < argc) { input_file = argv[++i]; }
		else if (strcmp(argv[i], "-o") == 0 && i + 1 < argc) { output_file = argv[++i]; }
		else if (strcmp(argv[i], "-v") == 0) { verbose = true; }
		else if (strcmp(argv[i], "-s") == 0) { struct_mode = true; }
		else if (strcmp(argv[i], "-t") == 0 || strcmp(argv[i], "-e") == 0) { text_mode = true; }
		else if (strcmp(argv[i], "--ms-sys") == 0 || strcmp(argv[i], "-m") == 0) { ms_sys_mode = true; }
		else {
	...
	if (count > BUFFER_MAX) {
		LOGE("Maximum Size is %d", BUFFER_MAX);
		return EXIT_FAILURE;
	}
#endif
	...
	if (!output_file) { output_file = "file.h"; }
	...
	if (!in) {
		LOGE("Opening input file '%s': %s", input_file, strerror(errno));
		FREE(sanitized);
		FREE(upper);
		return EXIT_FAILURE;
	}
	...
	if (count == (size_t)-1) {
		count = fpsize(in);
		if (count == (size_t)-1) {
			LOGE("Determining input file size: %s", strerror(errno));
			CLOSE_FILE(in);
			FREE(sanitized);
			FREE(upper);
			return EXIT_FAILURE;
	...
	if (!out) {
		LOGE("opening output file '%s': %s", output_file, strerror(errno));
		CLOSE_FILE(in);
		FREE(sanitized);
		FREE(upper);
		return EXIT_FAILURE;
	...
	fprintf_t(out,
		...
		" * Output File: %s\n"
		" * Bytes: %zu\n"
		" */\n\n",VERSION,xbasename(input_file),xbasename(output_file),memory_bytes);
	if (!ms_sys_mode) {
		fprintf_t(out, "#ifndef %s_H\n#define %s_H\n\n", upper, upper);
		fprintf_t(out, "#ifdef __cplusplus\nextern \"C\" {\n#endif\n\n");
		fprintf_t(out, "#define %s_SIZE %zu\n\n", upper, memory_bytes);
	}

	rewind(in); // Reset file pointer for second pass.

	if (ms_sys_mode) {
		fprintf_t(out, "unsigned char %s[] = {", sanitized);
	} else if (struct_mode) {
		fprintf_t(out,  "struct %s_struct {\n  unsigned char data[%s_SIZE];\n};\n\n", sanitized, upper);
		fprintf_t(out, "static const struct %s_struct %s = {\n	.data = {", sanitized, sanitized);
	} else {
		fprintf_t(out, "static const unsigned char %s[%s_SIZE] = {", sanitized, upper);
	}

	size_t total_bytes = 0;
	while ((read_bytes = fread(buffer, 1, (count > 0 ? (size_t)(count - total_bytes) : sizeof(buffer)), in)) > 0) {
		for (size_t i = 0; i < read_bytes; i++) {
			if (total_bytes > 0) fprintf_t(out, ",");
			if (total_bytes % BYTES_PER_LINE == 0) fprintf_t(out, "\n%s", struct_mode ? "		" : "	");
			fprintf_t(out, "0x%02X", buffer[i]);
			total_bytes++;
		}
	}
	...
	if (verbose)
		LOGI("Successfully written %zu bytes to %s", memory_bytes, output_file);
